...
class Contenedor {
    ...
    async readFile(){
        try {
            return JSON.parse(await fs.promises.readFile(`./persistencia/${this.fileName}.json`, 'utf-8'))
        } catch (error) {
            console.log(error)
        }
    }
    ...
    async writeFile(data){
        try {
            fs.promises.writeFile(`./persistencia/${this.fileName}.json`, JSON.stringify(data), 'utf-8')
        } catch (error) {
            console.log(error)
        }
    }
